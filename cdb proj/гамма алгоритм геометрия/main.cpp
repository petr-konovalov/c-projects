...
void drawPoly(const Polygon poly, const int dx = 0, const int dy = 0, const double M = 20)
{
...
void drawTriangulation(const Polygon poly, const Triangulation trian, const int dx = 0,
                       const int dy = 0, const double M = 20)
{
    for (int i = 0; i < trian.size(); ++i)
    {
        line((int)(poly[trian[i].a].x * M) + dx, getmaxy() - (int)(poly[trian[i].a].y * M) - dy,
             (int)(poly[trian[i].b].x * M) + dx, getmaxy() - (int)(poly[trian[i].b].y * M) - dy);
        line((int)(poly[trian[i].a].x * M) + dx, getmaxy() - (int)(poly[trian[i].a].y * M) - dy,
             (int)(poly[trian[i].c].x * M) + dx, getmaxy() - (int)(poly[trian[i].c].y * M) - dy);
        line((int)(poly[trian[i].b].x * M) + dx, getmaxy() - (int)(poly[trian[i].b].y * M) - dy,
             (int)(poly[trian[i].c].x * M) + dx, getmaxy() - (int)(poly[trian[i].c].y * M) - dy);
        Point mid = poly[trian[i].a] + (poly[trian[i].b] + poly[trian[i].c] - poly[trian[i].a] * 2) / 3;
        char str[30];
        itoa(i, str, 10);
        outtextxy((int)(mid.x * M) + dx - textwidth(str) / 2,
                  getmaxy() - (int)(mid.y * M) - dy - textheight(str) / 2, str);
    }
}
...
int main()
{
    ...
    trianglePolygon(poly, id, trian);
    for (int i = 0; i < trian.size(); ++i)
        cout << trian[i].a << ' ' << trian[i].b << ' ' << trian[i].c << endl;
    Graph g = getTriangleGraph(trian);
    ...
    vector<int> route = getRoute(g, trian, startPoint, finishPoint);
    for (int i = 0; i < route.size(); ++i)
        cout << route[i] << ' ';
    PolygonPair splitRes = splitPolygon(poly, trian, route, startPoint, finishPoint);
    */
    int n;
    cin >> n;
    Polygon poly(n);
    ...
    int startPointId, finishPointId;
    cin >> startPointId >> finishPointId;
    /*
    Polygon convexPoly = makeConvexPolygon(poly);
    Polygon inflateConvexPoly = inflateConvexPolygon(convexPoly, 1);
    Polygon extendedPolygon;
    Triangulation trian = triangleExternalRegion(poly, extendedPolygon);
    cout << trian.size();
    */
    Polygon extendedPolygon;
    Triangulation trian = triangleExternalRegion(poly, extendedPolygon);
    PolygonPair splitRes = spawnPolygon(poly, startPointId, finishPointId);
    //--------------------------
    int gd = DETECT;
    int gm;
    initgraph(&gd, &gm, "");
    ...
    drawPoly(poly, 50, 50);
    setcolor(3);
    drawPoly(poly, 320, 200);
    drawTriangulation(extendedPolygon, trian, 50, 50);
    */
    //drawPoly(inflateConvexPoly, 50, 50);
    //setcolor(3);
    //drawPoly(convexPoly, 50, 50);
    drawPoly(poly, 0, 250);
    drawTriangulation(extendedPolygon, trian, 300, 200);
    setcolor(4);
    drawPoly(splitRes.secondPoly);
    setcolor(3);
    drawPoly(splitRes.firstPoly);
    getchar();
    getchar();
    closegraph();
    //--------------------------
